              ...
              <table class="table table-bordered" id="rawData">
                ...
                <?php $state="SELECT * FROM users where id = '".$_SESSION['userId']."'";
                     ...
                     $data_details=mysqli_query($conn,"SELECT * FROM users INNER JOIN order_details 
                                                    ON  users.id = order_details.customer_id WHERE order_details.active = 1 AND order_details.status < 5 AND users.state IN ('".$state['state']."') " );
                    ...
                    <td>
                      <?php if($_SESSION['userRole'] == 4 && $row['status'] < 5) {  ?>
                        <a onclick="return confirm('Are you sure to approve ?')" href="invoicestatusupdate.php?idpro=<?php echo $row['id'];?>&st=<?php echo $row['status'];?>&idzm=<?php echo $_SESSION['userId'];?>" class="btn btn-warning">Send For Approve</a>
                      <?php }?>
                      
                      <a href="view_invoice.php?id=<?php echo $row['id']; ?>&date=<?php echo $dt->format('d-m-Y');?>" class="btn btn-success ">View</a>
                      <a href="delete_order.php?d_Order=1&amp;red=zonal_purchase_list.php" class="btn btn-danger" onclick="return confirm('Are you sure to delete')">Reject</a>
                    </td>
                     </tr>
